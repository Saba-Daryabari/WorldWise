...
function reducer(state,action){
  switch(action.type){
    ...
    case 'cities/deleted':
      return {...state,currentCity:false,cities:state.cities.filter((city)=>city.id!==action.payload),currentCity:{}}
    case 'rejected':
...
function CitiesProvider({children}) {
  const [{cities,isLoading,currentCity,err},dispatch]= useReducer(reducer,initialState)
      useEffect(function () {
    ...
    return<CitiesContext.Provider value={{cities,isLoading,currentCity,err,getCity,createCity,deleteCity}}>{children}</CitiesContext.Provider>
}
function useCities(){
